...
export default function QuantTradingPage() {
  ...
  const debouncedClick = useDebounce(500); // 0.5초 디바운스
  ...
  const [isExecutingTrade, setIsExecutingTrade] = useState(false); // 거래 실행 중 상태 추가
  const [currentTradeId, setCurrentTradeId] = useState(null); // 현재 거래 ID 추적
  const hasExecutedRef = useRef(false); // 실행 플래그 (useRef 사용)
  // 즉시 거래 모달 상태
  ...
  const handleButtonClick = () => {
    ...
    // 이미 거래 중이면 클릭 무시
    if (isTrading || countdownRef.current) {
      //console.log(' Button clicked but trade already in progress');
      return;
    }

    // 거래 가능 횟수 체크
    if (remaining <= 0) {
      //console.log(' Button clicked but daily limit reached');
      return;
    }

    //console.log(' Button click accepted, starting trade process');
    setIsGlowing(true);
    handleStart(); // 타이머만 시작
    // 반짝임 효과 1.5초 후 제거
  ...
  const debouncedHandleButtonClick = debouncedClick(handleButtonClick);
  ...
  const handleStart = () => {
    ...
    if (remaining <= 0) {
      setAlertInfo({
        title: t('quantTrading.error'),
        message: t("quantTrading.dailyLimitReached"),
        type: 'error'
      });
      setShowAlert(true);
      return;
    ...
    // 거래 중복 방지 - 이미 타이머가 실행 중이면 중단
    if (isTrading || countdownRef.current) {
      //console.log('Trade already in progress, ignoring duplicate start');
      return;
    }
    ...
    //console.log(` Starting trade countdown: ${delaySec} seconds`);

    setIsTrading(true);
    setRemainingTime(delaySec);
    hasExecutedRef.current = false; // 실행 플래그 초기화
    ...
    countdownRef.current = setInterval(() => {
      setRemainingTime(prev => {
        if (prev <= 1 && !hasExecutedRef.current) {
          hasExecutedRef.current = true; // 실행 플래그 설정
  ...
  const executeTrade = async () => {
    ...
    // 이미 다른 거래가 실행 중이면 중단
    if (currentTradeId && currentTradeId !== tradeId) {
      //console.log(' Another trade is already in progress, skipping');
      return;
    }
    ...
    try {
      ...
      //console.log(` Executing trade with amount: ${amt}`);
      ...
      //console.log(` Trade successful:`, res.data);
      ...
      setTradesToday(countRes.data.data.tradesToday || 0);
    } catch (err) {
  ...
  */}
      ...
      <button
        ...
        disabled={isTrading || remaining <= 0}
        className={`referra-butt ${isTrading || remaining <= 0
          ? "bg-gray-400 cursor-not-allowed opacity-50"
          : "bg-yellow-500 hover:bg-yellow-600 text-black"
          }`}
      >
        {isTrading
          ...
          : remaining <= 0
            ? t("quantTrading.dailyLimitReached")
